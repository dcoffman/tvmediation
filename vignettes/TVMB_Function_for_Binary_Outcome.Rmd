---
title: "Time Varying Mediation Function: Binary Outcome and Two Treatment Arms (Exposure Groups)"
author:
...
vignette: >
  %\VignetteIndexEntry{Time Varying Mediation Function: Binary Outcome and Two Treatment Arms (Exposure Groups)}
...
results_tvmb <- tvmb(treatment, t.seq, mediator, outcome, plot = TRUE, replicates = 500)
```
...
1.  `alpha_hat` the estimated main treatment arm (exposure group) of interest effect on mediator 
2.  `CI.lower.a` the lower limit of confidence intervals for coefficient alpha_hat
3.  `CI.upper.a` the upper limit of confidence intervals for coefficient alpha_hat
4.  `gamma_hat` the estimated main treatment arm (exposure group) of interest effect on outcome
5.  `CI.lower.g` the lower limit of confidence intervals for coefficient gamma_hat
6.  `CI.upper.g` the upper limit of confidence intervals for coefficient gamma_hat
7.  `beta_hat` the estimated effect of the mediator on the outcome
8.  `CI.lower.b` the lower limit of confidence intervals for coefficient beta_hat
9.  `CI.upper.b` the upper limit of confidence intervals for coefficient beta_hat
10. `tau_hat`    the estimated main treatment arm (exposure group) of interest effect on outcome, excluding adjustment for mediator
11. `CI.lower.t` the lower limit of confidence intervals for coefficient tau_hat
12. `CI.upper.t` the upper limit of confidence intervals for coefficient tau_hat
13. `medEffect` the time varying mediation effect - main treatment arm (exposure group) of interest on outcome (estimated via product method)

Optional returns based on arguments `CI = "boot"` passed by the user include:

14. `CI.low` the lower limit of confidence intervals of the time varying mediation effect `medEffect`
15. `CI.upper` the upper limit of confidence intervals of the time varying mediation effect `medEffect`

The above estimates are compiled in a single dataframe which can be accessed using `results_tvmb$Estimates`. The following line of code displays only the estimates at the first 6 time-points.

```{r}
head(results_tvmb$Estimates)
```

At each time point of interest `timeseq = t.seq`, the effects of the treatment (exposure) on mediator, exposure on outcome (adjusted and not adjusted for mediator) and mediator on outcome are estimated along with the respective 95% confidence intervals (CIs). The CIs are computed via a non-parametric bootstrap method (Efron and Tibshirani, 1986), drawing samples of size 1086 from the original sample with replacement, estimating the sample mean and then applying the percentile method to compute the 95% CIs. Note that the confidence intervals for the `alpha`, `gamma`, `beta` and `tau` coefficients `(alpha_hat, gamma_hat, beta_hat, tau_hat)` are computed regardless of the value of `CI` argument in the function. In the above results, `medEffect` is the estimated mediation effect of `varenicline` compared to `nicotine patch only`, that varies over `timeseq`. For `CI = "boot"` (which is the default option unless the user chooses otherwise) the 95% CI (`CI.low`, `CI.upper`) of `medEffect` is estimated via similar bootstrapping and percentile technique described earlier for the coefficients. 

If `plot = TRUE` argument is passed, the results will also include the following figures:

16. `plot1_a` plot for `alpha_hat` with 95% CIs across timeseq (`= t.seq`)
17. `plot2_g` plot for `gamma_hat` with 95% CIs across timeseq (`= t.seq`)
18. `plot3_b` plot for `beta_hat` with 95% CIs across timeseq (`= t.seq`)
19. `plot4_t` plot for `tau_hat` with 95% CIs across timeseq (`= t.seq`)
20. `MedEff` plot for `medEffect` across timeseq (`= t.seq`)
21. `MedEff_CI` plot for `medEffect` with 95% CIs across timeseq (`= t.seq`)
22. `bootstrap` plot for estimated `medEffect`(s) from bootstrapped samples across timeseq (`= t.seq`)

We recommend using the plots to interpret your findings as it may be difficult to derive meaning from the numerical values alone. To display the plots, use `results_tvmb$` followed by the name of the plot to access the required plot accordingly. For example:

```{r, fig.width = 10, fig.height = 5}
results_tvmb$plot1_a
```

In the above plot, the effect of `varenicline` on subjects' feeling of `negative mood in the last fifteen minutes` follows a mostly decreasing trend over time with occasional spikes, hovering between a range of -0.05 and 0.06; i.e. from a weak effect to an absence or some presence of an effect. The estimated 95% CI covers 0 (no effect).

```{r, fig.width = 10, fig.height = 5}
results_tvmb$plot2_g
```
The above plot shows the direct effect of `varenicline` on the outcome subjects' `daily smoking status` which follows a sinusoidal trend over time with occasional spikes around the end of weeks 1, 2 and 4. The estimated 95% CI covers 0 (no effect).

```{r, fig.width = 10, fig.height = 5}
results_tvmb$plot3_b
```

In the above plot, the effect of subjects' `negative mood in the last fifteen minutes` on the outcome subjects' `daily smoking status` follows a sinusoidal trend over time with occasional spikes around day 6-7 (end of week 1) and day 14 (end of week 3).

```{r, fig.width = 10, fig.height = 5}
results_tvmb$plot4_t
```

The above plot shows the total effect of `varenicline` on the outcome subjects' `daily smoking status` which follows a sinusoidal trend over time with occasional spikes around the end of days 4-6 and 14. The estimated 95% CI covers 0 (no effect).

```{r, fig.width = 10, fig.height = 5}
results_tvmb$MedEff
```

In the above plot, the effect of `varenicline` on `daily smoking status` that is mediated by the `negative mood in the last fifteen minutes` (using the product method of estimating mediation effect) shows that the effect  of `varenicline` on `daily smoking status` compared to `nicotine patch only` decreases rapidly in the first 2 weeks hovering mostly around 0 (no effect), then decreases sharply around day 14 before increasing again till day 21 and decreasing thereafter. The effect is extremely erratic and given that the confidence intervals cross zero for the estimated mediation effect, we can conclude that this effect is not statistically significant. See the below plot `MedEff_CI`.
...
This is the bootstrap plot showing the estimated mediation effect from bootstrapping the original sample 1000 times with 1086 sample size.

The `tvmb` function computes bootstrap confidence intervals by default. Therefore, the user may decide not to bootstrap CIs for the mediation effect by specifying `CI = "none"`. However if by mistake the user specifies `CI = "none"` and `replicates = 500` simultaneously, the function will not display an error, but simply execute without computing the CIs for mediation effect. Note that the CIs for the individual effects of exposure on mediator and mediator on outcome is computed even if the user passes the argument `CI = "none"`.
...
1. Baker, T. B., Piper, M. E., Stein, J. H., Smith, S. S., Bolt, D. M., Fraser, D. L., & Fiore, M. C. (2016). Effects of nicotine patch vs varenicline vs combination nicotine replacement therapy on smoking cessation at 26 weeks: A randomized clinical trial. JAMA, 315(4), 371-379. [doi:10.1001/jama.2015.19284](https://jamanetwork.com/journals/jama/fullarticle/2484340)

2. Efron, B. & Tibshirani, R. Bootstrap Methods for Standard Errors, Confidence Intervals, and Other Measures of Statistical Accuracy. Statist. Sci. 1 (1986), no. 1, 54--75. [doi:10.1214/ss/1177013815](https://projecteuclid.org/euclid.ss/1177013815)
