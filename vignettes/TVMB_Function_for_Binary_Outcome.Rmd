...
In the above plot, the effect of subjects' `negative mood in the last fifteen minutes` on the outcome `smoking status` follows a sinusoidal trend over time with occassional spikes around day 6-7 (end of week 1) and day 14 (end of week 3). 

```{r, fig.width = 10, fig.height = 5, echo = FALSE}
results_tvmb$MedEff
```

In the above plots, the effect of `varenicline` on `smoking status` that is mediated by the `negative mood in the last fifteen minutes` (using both the difference and product method of estimating mediation effect ) show that the effect follows no particular trend but oscilates between a weak and a positive effect compared to the effect of using `nicotine patch only`. The plot for mediation effect using the `difference method` shows that the effect  of `varenicline` on `smoking status` compared to `nicotine patch only` decreases rapidly in the first week and then increases around day 8 before decreasing till day 16 and increasing thereafter. The plot for mediation effect using the `product method` shows that the effect  of `varenicline` on `smoking status` compared to `nicotine patch only` decreases rapidly in the first 2 weeks hovering mostly around 0 (no effect), then decreases sharply around day 14 before sharply increasing again till day 21 and decreasing thereafter. The effect is extremely erratic and given that the confidence intervals cross zero for the estimated mediation effect (using product method), we can conclude that this effect is not statistically significant. See the below plot `MedEff_CI`.
