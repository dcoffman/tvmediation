...
8.  `hat.beta1` the estimated effect of treatment arm 1 (exposure group 1) on the outcome
9.  `CI.lower.beta1` the lower limit of confidence intervals for hat.beta1
10. `CI.upper.beta1` the upper limit of confidence intervals for hat.beta1
11. `hat.beta2` the estimated effect of treatment arm 2 (exposure group 2) on the outcome
12. `CI.lower.beta2` the lower limit of confidence intervals for hat.beta2
13. `CI.upper.beta2` the upper limit of confidence intervals for hat.beta2
14. `hat.beta3` the estimated effect of the mediator on the outcome
15. `CI.lower.beta3` the lower limit of confidence intervals for hat.beta3
16. `CI.upper.beta3` the upper limit of confidence intervals for hat.beta3
17. `hat.mediation1` the time varying mediation effect of treatment arm 1 (exposure group 1)
18. `hat.mediation2` the time varying mediation effect of treatment arm 2 (exposure group 2)

Optional returns based on arguments `CI = "boot"` include:

19. `CI.lower.NRT1` the lower limit of confidence intervals for the time varying mediation effect of treatment arm 1 (exposure group 1)
20. `CI.upper.NRT1` the upper limit of confidence intervals for the time varying mediation effect of treatment arm 1 (exposure group 1)
21. `CI.lower.NRT2` the lower confidence intervals for the time varying mediation effect of treatment arm 2 (exposure group 2)
22. `CI.upper.NRT2` the upper confidence intervals for the time varying mediation effect of treatment arm 2 (exposure group 2)
23. `SE_MedEff1` estimated standard error of the time varying mediation effect for treatment arm 1 (exposure group 2)
24. `SE_MedEff2` estimated standard error of the time varying mediation effect for treatment arm 2 (exposure group 2)

```{r}
results_tvma_3trt$Estimates
```

At each time point of interest `timeseq = t.est` which in this case is equal to `t.seq`, the effects of treatment (exposure) on mediator, treatment on outcome and mediator on outcome are estimated along with the respective 95% CIs. The CIs are computed via non-parametric bootstrap method \texttt{(Efron and Tibshirani, 1986)}, drawing samples of size 1086 from the original sample with replacement, estimating the sample mean and then applying the percentile method to compute the 95% CIs. Note that the confidence intervals for the `alpha` and `beta` coefficients `(hat.alpha.1, hat.alpha.2, hat.beta1, hat.beta2, hat.beta.3)` are computed regardless of the value of `CI` argument in the function. `hat.mediation1` and `hat.mediation2` are the estimated mediation effects of `NRT1 (varenicline)` and `NRT2 (comboNRT)` compared to the `placebo (nicotine patch only)` that varies over `t.est`. For `CI = "boot"` (which is the default option unless the user chooses otherwise) the standard errors of the estimated mediation effects and 95% CIs are estimated via similar bootstrapping technique described earlier for the coefficients. 

If `plot = TRUE` argument is passed, the results will also include the following figures:

25. `plot1_a1` plot for `hat.alpha1` with 95% CIs across `timeseq`
26. `plot2_a2` plot for `hat.alpha2` with 95% CIs across `timeseq`
27. `plot3_b1` plot for `hat.beta1` with 95% CIs across `timeseq`
28. `plot4_b2` plot for `hat.beta2` with 95% CIs across `timeseq`
29. `plot5_b3` plot for `hat.beta3` with 95% CIs across `timeseq`
30. `MedEff_NRT1` plot for `hat.mediation1` across `timeseq`
31. `MedEff_NRT2` plot for `hat.mediation2` across `timeseq`
32. `MedEff_CI_NRT1` plot for `hat.mediation1` with 95% CIs across `timeseq`
33. `MedEff_CI_NRT2` plot for `hat.mediation2` with 95% CIs across `timeseq`
...
results_tvma_3trt_new <- tvma_3trt(NRT1, NRT2, t.seq, mediator, outcome, t.est = c(0.2, 0.4, 0.6, 0.8), replicates = 500, grpname = "exposure", plot = TRUE)
```

```{r}
results_tvma_3trt_new$Estimates
```
Notice the name of the columns corresponding to the 95% CIs for `hat.mediation1` and `hat.mediation2` above. The following plots also reveal the change in axes and title names.

```{r, fig.width = 10, fig.height = 5}
results_tvma_3trt_new$MedEff_NRT1
results_tvma_3trt_new$MedEff_CI_NRT1
```
```{r, fig.width = 10, fig.height = 5}
results_tvma_3trt_new$MedEff_NRT2
results_tvma_3trt_new$MedEff_CI_NRT2
```
