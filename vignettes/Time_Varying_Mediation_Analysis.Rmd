...
1. `hat.alpha.1` the estimated main treatment arm (exposure group) of interest effect on mediator
2. `CI.lower.alpha1` the lower limit of confidence intervals for coefficient hat.alpha.1
3. `CI.upper.alpha1` the upper limit of confidence intervals for coefficient hat.alpha.1
4. `hat.beta.1` the estimated main treatment arm (exposure group) of interest effect on outcome
5. `CI.lower.beta1` the lower limit of confidence intervals for coefficient hat.beta.1
6. `CI.upper.beta1` the upper limit of confidence intervals for coefficient hat.beta.1
7. `hat.beta.2` the estimated mediator effect on outcome
8. `CI.lower.beta2` the lower limit of confidence intervals for coefficient hat.beta.2
9. `CI.upper.beta2` the upper limit of confidence intervals for coefficient hat.beta.2
10. `est.M` the time varying mediation effect - main treatment arm (exposure group) of interest on outcome

Optional returns based on arguments `CI = "boot"` include:

11. `boot.se.m` the estimated standard error of the time varying mediation effect `est.M`
12. `CI.lower` the lower limit of confidence intervals of the time varying mediation effect `est.M`
13. `CI.upper` the upper limit of confidence intervals of the time varying mediation effect `est.M`

```{r}
results$Estimates
```

At each time point of interest `t.est` which in this case is equal to `t.seq`, the effects of treatment (exposure) on mediator, treatment on outcome and mediator on outcome are estimated along with the respective 95% CIs. The CIs are computed via non-parametric bootstrap method \texttt{(Efron and Tibshirani, 1986)}, drawing samples of size 1086 from the original sample with replacement, estimating the sample mean and then applying the percentile method to compute the 95% CIs. Note that the confidence intervals for the `alpha` and `beta` coefficients `(hat.alpha.1, hat.beta.1, hat.beta.2)` are computed regardless of the value of `CI` argument in the function. `est.M` is the estimated mediation effect of `varenicline` compared to the other two treatment groups, that varies over `t.est`. For `CI = "boot"` (which is the default option unless the user chooses otherwise) the standard error of the estimated mediation effect and 95% CI is estimated via similar bootstrapping technique described earlier for the coefficients. 

If `plot = TRUE` argument is passed, the results will also include the following figures:

14. `Alpha1_CI` plot for `hat.alpha.1` with 95% CIs across timeseq
15. `Beta1_CI` plot for `hat.beta.1` with 95% CIs across timeseq
16. `Beta2_CI` plot for `hat.beta.2` with 95% CIs across timeseq
17. `MedEff` plot for `est.M` across timeseq
18. `MedEff_CI` plot for `est.M` with 95% CIs across timeseq
