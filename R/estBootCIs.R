...
estBootCIs <- function(trt, t.seq, M, Y, t.est, deltat, replicates) {
  ...
  #   est.smooth  -->   list of smoothed coefficients
  ...
  for(k1 in 1:replicates) {
    ...
    for (j in 2:length(t.seq)) {
      ...
      newMO.j.est.boot <- newMediatorOutcome(trt, temp.M.boot, temp.Y.boot[j - 1, ])
    ...
    storage.boot[k1, ] <- est.smooth.boot$true.M
  }
  ...
  # GENERATE OUTPUT
  results <- list(CI.upper = CI.all[2, ], CI.lower = CI.all[1, ])
  ...
  # par(mfrow <- c(1, 1))
  # plot(t.est, true.M, type="l")
  # lines(t.est, lower, type="l",col=2)
  # lines(t.est, upper, type="l",col=2)
